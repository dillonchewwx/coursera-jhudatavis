...
####PUT YOUR CODE HERE
q1_dat<-left_join(my_world_map, some_data_values)

ggplot(q1_dat, aes(x=long, y=lat, group=group)) + 
...
q2_world_map<-my_world_map %>%
    filter(region %in% c("Costa Rica", "Panama", "Nicaragua"))

q2_cities<-world.cities %>%
    filter(country.etc %in% c("Costa Rica", "Panama", "Nicaragua"), 
           pop>40000) %>%
    add_column(Measurement)

ggplot(q2_world_map, aes(x=long, y=lat, group=group)) + 
    geom_polygon(color="black",fill="white") + 
    geom_point(data=q2_cities, aes(x=long, y=lat, color=Measurement, group=NULL), size=5) + 
...
####PUT YOUR CODE HERE
q3_world_map<-my_world_map %>%
    filter(region %in% s_america$admin) 

q3_pop_est<-s_america %>%
    select(name_long, pop_est) %>%
    rename(region=name_long)

ggplot() + 
    geom_sf(data=q3_pop_est, aes(fill=pop_est)) +
