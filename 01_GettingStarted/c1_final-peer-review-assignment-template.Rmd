...
boxplot(dat$faminc_new, 
        main="Dillon's Boxplot for Family Income", 
        xlab="Family Income")
```
...
q7_dat<-dat %>%
    ...
    mutate(CC18_325a=recode(CC18_325a, `1`=1L, `2`=0L), 
           CC18_325d=recode(CC18_325d, `1`=1L, `2`=0L),
           tax_scale=CC18_325a+CC18_325d)
head(q7_dat$tax_scale, 20)
```
...
dat %>%
    group_by(region) %>%
    summarise(Trump_Approval_Mean=round(mean(CC18_308a), 2))
```
...
dat %>%
    filter(investor==2) %>%
    filter(faminc_new %in% c(5:10)) %>%
    summarise(Mean_Religion_Importance=mean(pew_religimp),
              Median_Religion_Importance=median(pew_religimp),
              Stdev_Religion_Importance=sd(pew_religimp))
```
...
```{r problem11}
q11_dat<-dat %>%
    summarise(Mean_FamInc=mean(faminc_new),
              Median_FamInc=median(faminc_new),
              Stdev_FamInc=sd(faminc_new))
kable(q11_dat)
```
...
qplot(data=dat, x=pid7, xlab="Seven Point Party ID", ylab="Count")
```